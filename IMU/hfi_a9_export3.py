...
#
# This Linux based file is edited from Gitee which is newer
# where data value is different from hfi_a9_export2.py
#
...
def handleSerialData(raw_data):
    ...
    if key < buff[2] + 5:  # 根据数据长度位的判断, 来获取对应长度数据
        ...
        # Output logged data
        outputDict = {
            'x-acc':acceleration[0] * -9.8 / acc_k, 
            'y-acc':acceleration[1] * -9.8 / acc_k, 
            'z-acc':acceleration[2] * -9.8 / acc_k,
            ...
            'x-magnetField':magnetometer[0], 
            'y-magnetField':magnetometer[1], 
            'z-magnetField':magnetometer[2]
        }
...
if __name__ == "__main__":
    ...
    result = pd.DataFrame(columns=features) # export as csv or json
    try:
        ...
        if not result.empty:
            ...
            new_name = "data/IMU-{}_{}.csv".format(today, n+1)
            result.to_csv(new_name)
            print(f'Saved data to {new_name}')
        else:
                ...
                try:
                    if buff_count > 0:
                        ...
                        if output_at_once.empty:
                            asdasf = 1 # This line to avoid indented block
                        else:
                            #print(f'output_at_once:{output_at_once}')
                            ...
                            #print(f'dictToResult:{dictToResult}')
                        
                            result = result.append(dictToResult, ignore_index=True)
