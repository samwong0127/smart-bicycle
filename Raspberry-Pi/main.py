...
import joblib
LR_filename_jb = 'RFmodel_F2_jb.sav'
LRF3_filename_jb = 'RFmodel_F3_jb_03-21-22.sav'
LRF3_DT_filename = 'DTmodel_F3_jb_03-28-22.sav'
UD_filename_jb = 'RFupdownModel_F2_jb.sav'
LRmodel = joblib.load(LRF3_filename_jb)
LRmodel_DT = joblib.load(LRF3_DT_filename)
UDmodel = joblib.load(UD_filename_jb)

# LED setup
from taillightControl import MAX7219
...
# Arduino connection
from ArdUnoConnection import setupComm, receiveFromArd, sendToArd
serialMotor = setupComm('/dev/ttyUSB1', 9600)
serialPedal = setupComm('/dev/ttyUSB2', 38400)
print("Serials are set")

# This file is compatible with python 2.7 in the given Rapsberry Pi 4B
...
bicycle_status_UP = 0 # 0 for horizontal # not used
...
featuresToLog = [
    ...
    'y-magnetField',
    'z-magnetField',
	'pedal_LX','pedal_LY','pedal_A','pedal_RX','pedal_RY'
]
...
if __name__ == "__main__":
    ...
    """
    baudrate = 921600 #original 921600

    resultToLog = pd.DataFrame()
        ...
        try:
            while True:
                try:
                    ...
                    if not resultToLog.empty:
                        n = sum(1 for f in os.listdir('data') if os.path.isfile(os.path.join('data', f)))
                        today = str(date.today())
                        new_name = "data/Log-{}_{}.csv".format(today, n+1)
                        resultToLog.to_csv(new_name)
                        print('Saved data to '+ str(new_name))
                    else:
                    ...
                    if buff_count > 0:
                        ...
                        if output_at_once.empty:
                            ...
                            if c == 0:
                                ...
                                Here
                                """
                                dfCur = pd.DataFrame.from_dict([dataCur])
                                new_df = dfCur
                                new_df['x-EDeg-1'] = dataPrev['x-EDeg']
                                new_df['y-EDeg-1'] = dataPrev['y-EDeg']
                                new_df['z-EDeg-1'] = dataPrev['z-EDeg']
                                #print("dataCur:")
                                #print(dataCur)
                                #print("dataPrev:")
                                #print(dataPrev)
                                ...
                                # Rearrange the features to match the model
                                new_df = new_df[[ 'x-acc', 'y-acc', 'z-acc', 'x-aVec', 'y-aVec', 
                                'z-aVec', 'x-EDeg','x-EDeg-1',  'y-EDeg', 'y-EDeg-1', 'z-EDeg','z-EDeg-1']]
                                #print(new_df)

                                # DO prediction
                                LR_pred = LRmodel_DT.predict(new_df)
                                #LR_pred = LRmodel.predict(new_df)
                                UD_pred = UDmodel.predict(new_df)

                                """
                                if dataCur['x-EDeg'] > 7:
                                    led.TurningRight(timeToSleep=0.1, iterations=2)
                                elif dataCur['x-EDeg'] < -7:
                                    led.TurningLeft(timeToSleep=0.1, iterations=2)
                                """
                                # Control LED taillight
                                if LR_pred[0] == 1:
                                ...
                                # Signal the motor
                                #if dataCur['y-EDeg'] > -35:
                                if UD_pred[0] == 1:
                                    print("Predicted Upslope")
                                    sendToArd(serialMotor, b''+str(1))
                                else:
                                    print("No slope")
                                    sendToArd(serialMotor, b''+str(0))

                                # Receive pedal data
                                pedalData = receiveFromArd(serialPedal)

                                now = datetime.now()

                                dictToResult = new_df
                                dictToResult['datetime'] = now

                                pedalDataList = pedalData.split(" ")
                                if len(pedalDataList) == 5:
                                    dictToResult['pedal_LX'] = pedalDataList[0]
                                    dictToResult['pedal_LY'] = pedalDataList[1]
                                    dictToResult['pedal_A'] = pedalDataList[2]
                                    dictToResult['pedal_RX'] = pedalDataList[3]
                                    dictToResult['pedal_RY'] = pedalDataList[4]
                                else:
                                    dictToResult['pedal_LX'] = -1
                                    dictToResult['pedal_LY'] = -1
                                    dictToResult['pedal_A'] = -1
                                    dictToResult['pedal_RX'] = -1
                                    dictToResult['pedal_RY'] = -1

                                resultToLog = resultToLog.append(new_df, ignore_index=True)

                            c += 1 # Cumulative for dataPrev and dataCur
